...
### Prebuilts
This feature runs the handler with a sample `GET` request to the corresponding pathname.

The result is then cached and will be cloned and returned later in request time
```ts
app.build('/', () => 'Hi');
```
